...
The main question that was supposed to be answered in this data mining task was whether or not the political opinion in Germany experienced a shift after the unification in the year 1990. This question was mainly to be answered in a datadriven fashion, i.e. by looking at rawdata and drawing conclusions directly from it rather than second hand sources and accounts towards this. Since this question is multifacetted and can be answered in various ways, the following analysis will focus on the following indicators which highlight things about the political climate in Germany.
...
Lastly, since this analysis is only concerned with Western-German opinion, we discard all subjects which say to reside in eastern German federal states. Furthermore, the data collected from West-Berlin was ommited for this analysis.

The remaining 21 variables were then used for all further analysis. The following diagram visualizes the process.

![](R Projects/ElectionPolling/plots/preprocessing.png)

## Modeling and Evaluation

The modeling was done in multiple parts.
...
The graph shows a stacked percentage barplot where the colors represent the mean political tendencies across the span of 7 years. A full account of left-right-leanings across the years can be found in *Appendix A*.

Additionally, to analyse how public sentiment deviates from a centrist point of view over time, Spearman's correlation coefficient @spearman1904proof was estimated and plotted below.
...
The plot shows, that while initially extremist positions persisted before and after unification, the trend is negative going further towards centrist positions. The correlation coefficient of *-0.47* indicates that their is a medium negative correlation between extremists position and the timeframe.
...
The following shows the regional difference in voting behavior for the years 1987 and 1993, three years before and after German reunification. The map data was taken from GADM @gadm2019.
...
The scale on the left shows the left-right barometer where 1 represents 1 hard-left stance, 6 represents a centrist position and 11 represents a hard-right stance. The Red-Blue shift clearly shows that almost all states with the exception of Saarland turned more left-leaning between these two years.
...
On the contrary side, if you are very unsatisfied with the *CDU* and hold the economy in high regards, a great percentage of people tends to vote for the liberal party *FDP* which was often the coalition partner of the *CDU*.

## Conclusion

The notion that political opinion after the fall of the Berlin wall has become more extreme is disproven insofar as political extremist tendencies have decreased over time, especially in the last decade. However, as Figure X and Figure Y show, there was still a strong sentiment of dissatisfaction during and directly after the events of 1989 and 1990. The plots show that not only did West Germany recover from the aftereffects of the reunion, the federal states are actually homogenising in their political opinions. While it is not clear if the recent shift towards a center-left position is due to the aftereffects of the GDR integration, it can be said that this shift appears to be stable for the moment.
